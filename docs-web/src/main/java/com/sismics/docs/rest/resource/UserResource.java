...
public class UserResource extends BaseResource {
    ...
    @POST
    @Path("register")
    public Response registerTemporaryUser(
        @FormParam("username") String username,
        @FormParam("password") String password,
        @FormParam("code") String validationCodeStr,
        @FormParam("remember") boolean longLasted){
        ...
        // Interact with the user data persistence layer
        UserDao userDao = new UserDao();
        JsonObjectBuilder response;
        ...
        try {
            ...
            System.out.println(e);
            if ("AlreadyExistingUsername".equals(e.getMessage())) {
                throw new ClientException("AlreadyExistingUsername", "Username is already in use", e);
            } else {
                throw new ServerException("UnknownError", "An unknown server error occurred", e);
            }
        }
